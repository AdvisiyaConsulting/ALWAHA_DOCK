...
class StudentPayslipLine(models.Model):
    ...
    type = fields.Selection(
        [("month", "Monthly"), ("year", "Yearly"), ("range", "Range"), ("sem1","Semester 1"),("sem2","Semester 2")],
        "Duration",
        required=True,
...
class StudentFeesStructureLine(models.Model):
    ...
    def create(self, vals):
        ...
        account=self.env['account.account'].sudo().search([("code","=","419100")])
        print("account %s" % str(account))
        if account:
            print("account %s"% str(account))
            vals["account_id"]=account.id

        return super(StudentFeesStructureLine, self).create(vals)
...
class StudentPayslip(models.Model):
    ...
    def create(self, vals):
        ...
        code="student.payslip"
        print("code",code)
        print("self.env['ir.sequence'].next_by_code(code)",self.env['ir.sequence'].next_by_code(code))
        if vals.get('number', 'New') == 'New':

            message = self.env['ir.sequence'].next_by_code("student.payslip")
            vals['number'] = message
    ...
    def action_move_create(self):
        ...
        for fees in self:
            print("feees****** %s" % str(fees.standard_id))
    ...
    def student_pay_fees(self):
        ...
        for rec in self:
            ...
            print("standard id %s"%str(rec.standard_id))
            vals = {
...
class AccountMove(models.Model):
    ...
    def action_check(self):
        moves =self.env['account.move'].sudo().search([("payment_state","!=","paid"),
                                                       ("amount_residual",">",0)])

        print("movesss",moves)
        for rec in moves:
            if rec.invoice_date_due < date.today():
                payslip_id=rec.student_payslip_id.id
                base_url = str(self.env['ir.config_parameter'].sudo().get_param('web.base.url'))

                url=base_url+"/web#id="+str(payslip_id)+"&cids=2&menu_id=245&model=student.payslip&view_type=form"
                _logger.info("messagee iff %s" %str(url))
                users = []
                activity_type_id = self.env.ref('mail.mail_activity_data_todo')
                activity_obj = self.env['mail.activity']
                note= "We kindly request that you review the status of this invoice due date: %s." % (
                ...
                exist_activity=activity_obj.sudo().search([('res_model_id',"=",self.env['ir.model'].sudo()._get('account.move').id),
                                                           ('note',"=",note)])
                print("exist activity %s" %str(exist_activity))
                #if  exist_activity :
                users += self.env.ref('school.group_school_administration').sudo().users
                for user in users:
                        vals = {
                            'activity_type_id': activity_type_id.id,
                            'note': "We kindly request that you review the status of this invoice due date: %s." % (rec.invoice_date_due),
                            'user_id': user.id,
                            'res_id': rec.id,
                            'date_deadline': date.today(),
                            'res_model': 'account.move',
                            'res_model_id': self.env['ir.model'].sudo()._get('account.move').id,
                        }
                        body = (
                                """
                            <div dir="rtl" >
                                ...
                                + """
                                                    <br/><br/>
                                                    :نرجو منك مراجعة حالة هذا اﻹيصال"""
                                + """<br/>
                                                       <a href=""" + str(url) + """" style="color: red;">""" + str(
                            rec.student_payslip_id.name) + """</a>"""

                                + """<br/><br/>للطالب:"""
                                + str(student_name)
                                + """<br/><br/>تاريخ الاستحقاق:"""
                                +str (rec.invoice_date_due)
                        ...
                        if not exist_activity:
                            ...
                            _logger.info("activity %s" %str(activity))
                        email_values={
                                "email_from": self.env.user.email or "",
                                "email_to": user.email,
                                "subject": "دفعة طالب متأخرة",
                                "body_html": body,
                            }
                        # send email
                        self.send_email_with_template(rec.student_payslip_id.student_id.user_id.id, "دفعة طالب متأخرة", email_values)
                        #self.send_email(user.email, "Overdue Payment", note)



        # for move in moves :
        #     if move.invoice_date_due > date.today():
        #         action = self.env.ref('school_fees.view_student_payslip_form')
        #         print("self.student_payslip_id", self.student_payslip_id)
        #         notification = {
        ...
        #                 'message': '%s',
        #                 'links': [{
        #                     'label': self.student_payslip_id.name,
        #                     'url': f'#action={action.id}&id={self.student_payslip_id.id}&model=student.payslip'
        #                 }],
        #                 'sticky': True,
        #             }
        #         }
    ...
    def send_email_with_template(self, recipient_email, template_name, email_values):
        _logger.info("langg if templatee**************** %s" % str(self.env.user.lang))
        _logger.info("emaiil name if templatee**************** %s" % str(_(template_name)))
        ...
        if template:
            ...
            template.send_mail(self.student_payslip_id.student_id.id, force_send=True, email_values=email_values )
            return True
        else:
            return False
