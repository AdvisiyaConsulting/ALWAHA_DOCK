...
class AcademicYear(models.Model):
    ...
    def validate_year(self,year):
        # Regular expression pattern to match four digits
        pattern = r'^\d{4}$'
        _logger.info('add year constrant %s' %str(pattern))
    ...
    def _check_code(self):
        for record in self:
            ...
            if not self.validate_year(record.code):
                raise ValidationError(_("Invalid year format. Please enter a year in the format 'xxxx'."))
...
class SchoolStandard(models.Model):
    ...
    @api.depends(
        "standard_id", "school_id", "division_id", "school_id"
    )# remove "medium_id"
    ...
    capacity = fields.Integer("Total Seats", compute='compute_capacity', readonly=False,store=True, help="Standard capacity")
    ...
    class_room_id = fields.Many2one(
        "class.room", "Room Number", help="Class room of the standard",required=True,
    )
    ...
    def check_seats(self):
        ...
        if self.capacity > self.class_room_id.capacity :
            raise ValidationError(_("Total seats should be less than capacity of class room !"))
    ...
    def compute_capacity(self):
        for record in self:
            print("Changed class room id ", record.class_room_id.capacity)
            record.capacity = record.class_room_id.capacity
    ...
    def write(self, vals):
        if 'school_id' in vals:
            ...
            if school:
                ...
                self.cmp_id = school.company_id

        return super(SchoolStandard, self).write(vals)
...
class SchoolSchool(models.Model):
    ...
    documents = fields.One2many(
        "documents.type", "school_id", "Documents", help="School documents"
    )
...
class StudentDocument(models.Model):
    ...
    description = fields.Many2one('documents.type',"Name", help="Description of document")
    document = fields.Binary(
        "Legal Document", required=True, help="Student Document"
    )
class TypeDocument(models.Model):
    """Defining type documents"""

    _name = "documents.type"
    _description = "Document Type"
    _rec_name ="description"
    description = fields.Char("Description", help="Description of type of docuemnt")
...
class ClassRoom(models.Model):
    ...
    floor_id = fields.Many2one(
        'school.floor',
        'floor',
        help="Select the floor of the classroom",
    )
    inspection_ids = fields.One2many(
        "school.inspection",
        "class_room_id",
        "Class room inspection",
        help="Class room inspection",
    )



class SchoolInspection(models.Model):
    _name = "school.inspection"
    _description = "School Inspection"
    name = fields.Char("Name", help="Inspection name")
    description = fields.Text("description of the observation")
    inspection_date = fields.Date("Inspection date")
    attachment = fields.Binary(
        "Attachment", help="Attachments"
    )
    class_room_id = fields.Many2one(
        "class.room", "Room Number", help="Select the floor"
    )
    floor_id = fields.Many2one(
        'school.floor',
        'Floor',
        help="Select the floor",
    )
    type = fields.Selection(
        [("floor", "Floor"), ("class_room", "Class room")],
        "Inspection Type",
        required=True,
...
class Report(models.Model):
    ...
    def render_template(self, template, values=None):
        ...
        if student_rec and student_rec.state == "draft":
            raise ValidationError(
                _("You cannot print report for student in unconfirm state!")
            )
        return super(Report, self).render_template(template, values)

class Floor(models.Model):
    """Defining floor informations"""

    _name = "school.floor"
    _description = "Floor"

    name = fields.Char("Name", help="Floor name")
    number = fields.Char("Floor Number", help="Floor number")
    class_room_ids = fields.One2many(
        "class.room",
        "floor_id",
        "Class room in floor",
        help="Class rooms which are in this floor",
    )
    supervisor_id = fields.Many2one(
        'school.admin',
        'Supervisor',
        help="Select the supervisor associated with this floor",
    )
    inspection_ids = fields.One2many(
        "school.inspection",
        "floor_id",
        "Floor inspection",
        help="Floor inspection",
    )
    ...
    def onchange_supervisor_id(self):
        if self.supervisor_id:
            ...
            self.supervisor_id.floor_id = self._origin.id
