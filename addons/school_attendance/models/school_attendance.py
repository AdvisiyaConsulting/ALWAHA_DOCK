...
class DailyAttendance(models.Model):
    ...
    teacher_ids = fields.One2many(
          "daily.attendance.line",
        "standard_id",
        help="Teachers teaching at this time",
    )
    _sql_constraints = [
        (
            "attend_unique",
    ...
    def check_attendance_unique(self):
        """Method to check unique attendance."""
        for record in self:
            # Check if there are any records with the same start_time, end_time, and date
    ...
    daily_attendance_type=fields.Selection([("teacher", "Teacher"), ("student", "Student")],
        "Daily Attendance Type",

        help="Type of Attendance",
    )

    def get_timetable_line(self):
        # Example date
        ...
        # Convert weekday number to actual name
        weekday_name = date_obj.strftime('%A').lower()
        _logger.info("weekday_nam %s"%str(weekday_name))
        _logger.info("self start date %s"%str(self.start_time))

        teacher_ids =[]
        time_table_lines = self.env['time.table.line'].sudo().search([("week_day", "=", weekday_name),
                                                                ("start_time","=",self.start_time),
                                                                ("end_time","=",self.end_time)])

        _logger.info("time_table_lines %s"%str(time_table_lines))

        for time_table in time_table_lines:
            teacher_ids.append(time_table.teacher_id.id)
            _logger.info('time_table %s' % str(time_table))
            _logger.info('time_table %s' % str(time_table.week_day))
            self.env['daily.attendance.line'].sudo().create({
                "teacher_id": time_table.teacher_id.id,
                "standard_id":self.id,
                "is_present":True,
        ...
        _logger.info('teacher %s' %str(teacher_ids))
    def do_regenerate(self):
        self.is_generate = False
        self.student_ids = False
        self.teacher_ids=False

    def attendance_teacher_validate(self):
        self.state ="validate"
...
class DailyAttendanceLine(models.Model):
    ...
    stud_id = fields.Many2one("student.student", "Student Name")
    ...
    def create(self, vals):
        if vals.get("student_id"):
            vals.update(self._update_vals(vals.get("student_id")))
        if vals.get("stud_id"):
            stud_id = vals['stud_id']
            student_id = self.env['student.student'].sudo().search([('id', '=', stud_id)])

            vals['name'] = student_id.name
        if vals.get('teacher_id'):
            teacher_id = vals['teacher_id']
            teacher=self.env['school.teacher'].sudo().search([('id','=',teacher_id)])
            if teacher:
                vals['name']=teacher.name

        return super(DailyAttendanceLine, self).create(vals)

    def write(self, vals):
        if vals.get("student_id"):
            vals.update(self._update_vals(vals.get("stud_id")))
        if vals.get("stud_id"):
            stud_id=self.env['student.student'].sudo().search([('id','=',vals['stud_id'])])
            self.name=stud_id.name
        if vals.get("teacher_id"):
            teacher=self.env['school.teacher'].sudo().search([('id','=',vals['teacher_id'])])
            if teacher :
                self.name=teacher.name


        return super(DailyAttendanceLine, self).write(vals)
